...
unset($item);
usort($items, function ($a, $b) use ($timeline_role) {
    return strcmp($b['year'], $a['year']) ?: (($timeline_role($b) === 'merge') <=> ($timeline_role($a) === 'merge')) ?: $a['_order'] <=> $b['_order'];
});
$years = array_values(array_unique(array_map(function ($item) {
    return substr($item['year'], 0, 4);
}, $items)));
$latest = $items[0]['year'];
?>
...
>
    <div class="mc-world-backdrop" aria-hidden="true"></div><canvas class="mc-world-canvas" aria-hidden="true"></canvas>
    ...
    <div class="mc-stops-stream">
        ...
        foreach ($items as $index => $item) :
            ...
            if ($year !== $previous) :
                ...
                $previous = $year;
        ?><div class="mc-year-group" id="mc-year-<?php echo esc_attr($year); ?>">
                    <header class="mc-year-heading"><span><?php echo esc_html($year); ?></span></header><?php endif; ?>
                <section class="mc-milestone-block <?php echo $featured ? 'has-photo' : ''; ?>" data-origin="<?php echo esc_attr($origin); ?>" data-side="<?php echo $origin === 'vanilla' || (!$origin && $index % 2) ? 'right' : 'left'; ?>" data-year="<?php echo esc_attr($year); ?>" data-date="<?php echo esc_attr($item['year']); ?>" id="mc-stop-<?php echo (int)$index; ?>">
                    ...
                    <article class="mc-gui-card">
                        <h3><button class="mc-entry-title" type="button" data-detail="mc-detail-<?php echo (int)$index; ?>" aria-label="阅读：<?php echo esc_attr($item['title']); ?>"><?php echo esc_html($item['title']); ?></button></h3>
                        <div class="mc-card-body"><?php echo wp_kses_post(wpautop($item['summary'])); ?></div>
                        <button class="mc-chest-btn" type="button" data-detail="mc-detail-<?php echo (int)$index; ?>" aria-label="打开回忆：<?php echo esc_attr($item['title']); ?>"><img src="<?php echo esc_url($assets . '/images/ender-chest.png'); ?>" alt=""><span class="mc-chest-tip">打开回忆</span></button>
                    </article>
                    <?php if ($featured) : $caption = $featured['name'] ?: $item['title']; ?><figure class="mc-memory-display"><button type="button" data-detail="mc-detail-<?php echo (int)$index; ?>" data-photo="true" aria-label="查看特色图片：<?php echo esc_attr($caption); ?>"><img src="<?php echo esc_url($featured['url']); ?>" alt="<?php echo esc_attr($caption); ?>" width="<?php echo (int)$featured['width']; ?>" height="<?php echo (int)$featured['height']; ?>" loading="lazy" decoding="async"></button>
                            <figcaption><?php echo esc_html($caption); ?><span><?php echo esc_html($item['year']); ?></span></figcaption>
                        </figure><?php endif; ?>
                    <template id="mc-detail-<?php echo (int)$index; ?>">
                        <h2><?php echo esc_html($item['title']); ?></h2><time><?php echo esc_html($item['year']); ?></time>
                        <div class="mc-entry-text"><?php echo wp_kses_post($item['content_html']); ?></div><?php foreach ($detail_photos as $photo_index => $photo) : ?><figure <?php echo $photo_index === 0 ? 'data-featured-page' : ''; ?>><img width="<?php echo (int)$photo['width']; ?>" height="<?php echo (int)$photo['height']; ?>" src="<?php echo esc_url($photo['url']); ?>" alt="<?php echo esc_attr($photo['name'] ?: $item['title']); ?>">
                                <figcaption><?php echo esc_html($photo['name'] ?: $item['title']); ?> · SDUcraft 历史档案</figcaption>
