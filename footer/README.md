...
- `layers` 从后台世界设置读取，固定顺序为天空、草地、泥土、石头、深板岩、基岩。
...
后台「自定义页脚 → 世界设置」可调整地层厚度、橡树／树苗开关及挖掘速度倍率。默认厚度依次为 6、1、3、8、10、1，装饰默认开启，速度默认 1 倍。配置读取及范围校验位于 `settings.php`。

草地和基岩至少一层；泥土、石头、深板岩、空气可设为零。橡树需要至少六层空气，树苗需要一层。速度倍率范围为 0.25–4，倍率越高挖掘时间越短，不改变方块硬度比例。

彩蛋深度范围按保存的地层计算，修改后刷新设置页可看到新范围。越界固定坐标会限制到世界边界，再按原规则寻找可挖掘方块。随机深度随世界高度变化；指定方块不存在时仍使用附近可挖掘方块。

## 草地类方块挖掘音效

`blocks.json` 的 `hitSound` 支持单个路径或路径数组。草方块配置 Grass_step1–6，开始挖掘即随机播放，持续挖掘每约 200ms 播放一次，音量为主音量的 25%，半速播放并关闭保调以降低音调。松手、取消、页面隐藏或挖开时停止过程音效；挖开后仍随机播放 Grass_dig1–4。
这是对 Java Edition 挖掘音效节奏的网页近似，播放器复用且不补播卡顿期间错过的节拍。

橡树树叶和树苗使用同一套 Grass_step1–6 与 Grass_dig1–4。音频播放器按资源地址去重，多个方块共用这些声音不会新增音频资源或播放器。
